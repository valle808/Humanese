...
const placeuserStatistics = () => {
  ...
  if (profilePicture) profilePicture.src = userData.profileImage;

  // Set status from session storage
  const currentStatusDisplay = document.querySelector(".parrot-status");
  if (currentStatusDisplay && userData.statusOption !== undefined) {
    currentStatusDisplay.textContent = userData.statusOption;
  }
}
...
const setupStatusButtons = () => {
  const statusButtons = document.querySelectorAll(".classcontainstatus button");
  const currentStatusDisplay = document.querySelector(".parrot-status");
  const clearStatusBtn = document.querySelector(".clearspan");

  if (statusButtons.length > 0 && currentStatusDisplay) {
    statusButtons.forEach(btn => {
      btn.addEventListener("click", () => {
        const icon = btn.textContent;
        currentStatusDisplay.textContent = icon;

        let userData = getUserDataFromSessionStorage();
        if (userData) {
          userData.statusOption = icon;
...
function renderLeaderboard(profileList) {
  ...
  profileList.forEach((entry, index) => {
    ...
    let entryId = entry?.id || entry?.userId;
    if (userId && userId == entryId) {
