...
const setIndexSiteLanguage = (path, language) => {
    fetch(path)
        ...
        .then(data => {
            ...
            // Re-initialize UI effects after translation
            if (window.initializeUIEffects) window.initializeUIEffects();
        })
...
document.addEventListener('DOMContentLoaded', () => {
    const langButton = document.querySelector('.site-language-button');
    const langDropdown = document.querySelector('.hover-site-language');
    const langOptions = document.querySelectorAll('.available-lang');
    ...
    if (langButton && langDropdown) {
        ...
        langOptions.forEach(option => {
            option.addEventListener('click', (e) => {
                ...
                if (selectedLang) {
                    setIndexSiteLanguage(`assets/JSON/landing-${selectedLang}.json`, selectedLang);
                }
            });
        });
    }
