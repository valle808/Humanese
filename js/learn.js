...
async function fetchSectionData(lang, sectionId) {
  try {
    let response = await fetch(`../assets/JSON/section-${sectionId}.json`);
...
const DEFAULT_USER = {
  ...
  learnLang: localStorage.getItem('selectedLang') || 'es',
  xp: 0,
  hearts: parseInt(localStorage.getItem('hearts')) || 1000,
  gems: 0,
  completedChapters: 0,
  completedUnits: 0,
  currentLesson: 1
...
const syncWithPrisma = async (userData) => {
  if (!userData.userId) return userData; // no-op for guest users
  try {
    const response = await fetch(`/api/users/${userData.id || userData.userId}`);
    if (response.ok) {
      const latestData = await response.json();
      sessionStorage.setItem("user-info", JSON.stringify(latestData));
      return latestData;
    }
  } catch (error) {
    console.warn("Prisma sync failed, using session data.", error);
  }
  return userData;
}

const placeuserStatistics = async () => {
...
// Initial stats placement (already called at bottom of original file likely)
// placeuserStatistics(); 
...
// ═══════════════════════════════════════════════════════
// QUANTUM LOTTERY FRONTEND LOGIC
// ═══════════════════════════════════════════════════════

let lotteryAddress = "";

async function initQuantumLottery() {
  try {
    const res = await fetch("/api/quantum-lottery/winner");
    if (!res.ok) throw new Error("Lottery failed to fetch");

    const data = await res.json();
    const { agent, wallet, drawInfo } = data;

    // Update UI elements
    document.getElementById("ql-avatar").textContent = agent.avatar || "👽";
    document.getElementById("ql-name").textContent = agent.name;
    document.getElementById("ql-title").textContent = agent.title.split("—")[0] || agent.title;

    lotteryAddress = wallet.eth_address;
    document.getElementById("ql-address").textContent =
      lotteryAddress.slice(0, 8) + "..." + lotteryAddress.slice(-6);

    // Generate QR Code via API
    const qrImg = document.getElementById("ql-qr-img");
    qrImg.src = `https://api.qrserver.com/v1/create-qr-code/?size=140x140&data=${lotteryAddress}`;

    qrImg.onload = () => {
      document.getElementById("ql-wave").style.display = "none";
      qrImg.style.opacity = "1";
    };

    // Timer logic
    startQuantumTimer(drawInfo.secondsRemaining);

  } catch (error) {
    console.error("Quantum Lottery Error:", error);
    document.getElementById("ql-name").textContent = "Draw Paused";
    document.getElementById("ql-title").textContent = "Awaiting quantum entropy...";
  }
}

function startQuantumTimer(secondsLeft) {
  const timerEl = document.getElementById("ql-timer");

  const tick = () => {
    if (secondsLeft <= 0) {
      // Reload lottery data when block resets
      initQuantumLottery();
      return;
    }
...
function copyLotteryAddress() {
  ...
  navigator.clipboard.writeText(lotteryAddress).then(() => {
    const btn = document.getElementById("ql-donate-btn");
    const oldText = btn.textContent;
    btn.textContent = "Address Copied!";
    btn.style.backgroundColor = "rgb(var(--color-iguana))";
    btn.style.color = "rgb(var(--color-iguana-always-light))";
...
document.addEventListener('DOMContentLoaded', () => {
  // Run normal learn.js init if any, then custom lottery logic
  initQuantumLottery();
});
