...
export default function NeuralDossierPage({ params }: { params: { id: string } }) {
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-12 text-center space-y-12">
        ...
        <h1 className="text-5xl font-black uppercase italic tracking-tighter text-white">Signature_Lost.</h1>
        <p className="text-xl text-white/40 italic font-light max-w-xl">{error || 'The requested neural signature does not exist in the OMEGA registry.'}</p>
        <Link href="/atlas" className="px-12 py-6 bg-white/5 border-2 border-white/10 rounded-full text-[11px] font-black uppercase tracking-[0.6em] text-white/40 hover:bg-white/10 hover:text-white transition-all italic leading-none active:scale-95">
           Return to Atlas
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-primary/40 selection:text-white font-sans overflow-hidden relative">
      ...
      <header className="relative z-50 w-full px-8 lg:px-14 py-8 flex justify-between items-center border-b border-white/5 bg-black/40 backdrop-blur-3xl">
        <Link href="/atlas" className="inline-flex items-center gap-6 text-white/40 hover:text-primary transition-all text-[11px] font-black uppercase tracking-[0.6em] group italic active:scale-95 leading-none">
      ...
      <main className="relative z-10 max-w-[1600px] mx-auto px-8 md:px-16 lg:px-24 pt-16 lg:pt-32 space-y-16 lg:space-y-32 h-[calc(100vh-100px)] overflow-y-auto no-scrollbar pb-40">
        ...
        >
          <div className="space-y-12 max-w-5xl">
            <div className="inline-flex items-center gap-6 px-8 py-3 bg-white/5 border border-white/10 rounded-full backdrop-blur-3xl shadow-lg">
            ...
            <div className="space-y-8">
              <h1 className="text-7xl md:text-[11rem] font-black uppercase tracking-tighter italic leading-[0.8] text-white">
                {data.label}<br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-white/80 to-primary/30">{data.group}.</span>
              </h1>
              <p className="text-2xl md:text-3xl text-white/40 max-w-4xl leading-relaxed font-light italic tracking-tight">
                Detailed neural imprint for <span className="text-white/80">UID_{data.id}</span>. This entity represents a core shard in the Sovereign intelligence mesh.
              </p>
            </div>
          </div>

          <div className="flex gap-10 items-center shrink-0 w-full lg:w-auto">
               <div className="p-12 bg-black border-2 border-white/5 rounded-[4rem] text-center space-y-6 backdrop-blur-3xl group hover:border-primary/40 transition-all shadow-2xl shadow-inner flex flex-col justify-center min-h-[200px] w-full lg:w-auto">
                  <div className="text-6xl font-black text-white italic tracking-tighter leading-none group-hover:text-primary transition-colors">{data.connections?.length || 0}</div>
                  <div className="text-[10px] text-white/20 font-black uppercase tracking-[0.5em] italic leading-none">Neural_Links</div>
               </div>
          </div>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-12 gap-16 items-start">
          ...
          <div className="lg:col-span-7 space-y-16">
            ...
            <section className="bg-black border-2 border-white/5 rounded-[4rem] overflow-hidden group backdrop-blur-3xl shadow-2xl shadow-inner relative">
                ...
                <div className="p-12 lg:px-16 border-b-2 border-white/5 flex justify-between items-center bg-white/[0.02] relative z-10">
                    <h2 className="flex items-center gap-8 font-black uppercase tracking-tight text-4xl italic text-white/40 leading-none pl-2">
                ...
                <div className="p-12 lg:p-16 grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                    {Object.entries(data.metadata || {}).map(([key, value]: [string, any]) => (
                      <div key={key} className="p-10 bg-white/[0.02] border-2 border-white/5 hover:border-primary/40 rounded-[3rem] group/meta transition-all shadow-inner">
                         <div className="text-[10px] font-black text-white/20 uppercase tracking-[0.5em] italic leading-none mb-4 group-hover/meta:text-primary transition-colors">{key}</div>
                         <div className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none break-words">
            ...
            <section className="bg-black border-2 border-white/5 rounded-[4rem] p-16 lg:p-24 backdrop-blur-3xl text-center space-y-12 shadow-2xl relative overflow-hidden group shadow-inner">
                 ...
                 <h3 className="text-6xl font-black uppercase italic tracking-tighter relative z-10 leading-none text-white/90">Integrity_Verified.</h3>
                 ...
                 <div className="pt-10 flex flex-wrap justify-center gap-10 relative z-10">
                    <button className="px-14 py-7 bg-white/5 border-2 border-white/10 rounded-full text-[11px] font-black uppercase tracking-[0.5em] transition-all hover:bg-white hover:text-black hover:border-white italic leading-none active:scale-95 shadow-2xl">Re-Audit_Signature</button>
          ...
          <div className="lg:col-span-5 space-y-16">
            ...
            <section className="bg-black border-2 border-white/5 p-12 lg:p-16 rounded-[4rem] backdrop-blur-3xl space-y-12 group hover:border-primary/40 transition-all relative overflow-hidden shadow-2xl shadow-inner">
                <div className="absolute top-0 right-0 p-12 opacity-[0.03] group-hover:scale-125 transition-transform duration-3000">
                   <Orbit size={200} className="text-primary" />
                </div>
                <h2 className="text-[11px] font-black uppercase tracking-[1em] text-white/20 italic flex items-center gap-8 pl-4 leading-none relative z-10">
                ...
                <div className="space-y-6 relative z-10">
                    {data.connections?.map((conn: any) => (
                      <Link
                        ...
                        className="flex items-center justify-between p-8 rounded-[2.5rem] bg-white/[0.02] border-2 border-white/5 hover:border-primary/30 transition-all group/conn shadow-inner"
                      >
                        <div className="flex items-center gap-8">
                           <div className="w-16 h-16 bg-white/5 border-2 border-white/10 rounded-2xl flex items-center justify-center text-white/20 group-hover/conn:text-primary group-hover/conn:border-primary/30 transition-all shadow-xl">
                              {conn.group === 'AGENT' ? <Cpu size={28} /> : conn.group === 'USER' ? <Target size={28} /> : <Database size={28} />}
                           </div>
                           <div className="space-y-2">
                              <div className="text-2xl font-black text-white/60 group-hover/conn:text-white uppercase tracking-tighter italic transition-colors leading-none">{conn.label}</div>
                              <div className="text-[10px] text-white/10 font-black italic uppercase tracking-[0.4em] leading-none">{conn.group} // LINKED_NODE</div>
                           </div>
                        </div>
                        <ArrowUpRight size={24} className="text-white/10 group-hover/conn:text-primary transition-all" strokeWidth={3} />
                      </Link>
                    ))}
                    {(!data.connections || data.connections.length === 0) && (
                      <div className="p-12 text-center text-[12px] font-black uppercase tracking-[0.8em] text-white/10 italic leading-none bg-white/[0.01] rounded-[3rem] border-2 border-dashed border-white/5">
            ...
            <section className="bg-primary/5 border-2 border-primary/30 rounded-[4rem] p-12 lg:p-16 flex flex-col items-center gap-12 shadow-2xl relative overflow-hidden group shadow-inner">
                 ...
                 <div className="text-center space-y-6 relative z-10">
                    ...
                    <p className="text-2xl text-white/30 font-light leading-relaxed italic mx-auto max-w-sm tracking-tight">"All neural imprints are transient and subject to consensus re-evaluation."</p>
                 </div>
