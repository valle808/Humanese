      ...
      <main className="relative z-10 max-w-[1700px] mx-auto px-8 pt-24 lg:pt-32 space-y-32 flex-1 flex flex-col">
        ...
        <div className="grid lg:grid-cols-12 gap-16 items-start">
           ...
           <div className="lg:col-span-4 space-y-12 lg:sticky lg:top-32 h-fit">
              <div className="bg-background border-2 border-primary/20 p-12 rounded-[5rem] backdrop-blur-3xl space-y-12 shadow-[0_60px_120px_rgba(var(--primary),0.1)] relative overflow-hidden group transition-all">
                ...
                <div className="space-y-8 relative z-10 pl-2">
                   ...
                   <div className="flex flex-wrap gap-4">
                      <div className="px-6 py-2 bg-muted/10 border border-border rounded-full text-[10px] font-black text-muted-foreground/20 uppercase tracking-[0.4em] italic">{strategy.type}</div>
                      <div className="px-6 py-2 bg-primary/10 border border-primary/20 rounded-full text-[10px] font-black text-primary uppercase tracking-[0.4em] italic leading-none flex items-center gap-3">
              ...
              <div className="bg-background border-2 border-border p-12 rounded-[5rem] backdrop-blur-3xl space-y-12 shadow-[0_60px_120px_rgba(0,0,0,0.95)] group hover:border-primary/30 transition-all relative overflow-hidden shadow-inner">
                 ...
                 <div className="space-y-8 relative z-10 pl-2">
                    ...
                    <div className="p-8 bg-muted border-2 border-border rounded-[2.5rem] space-y-4 shadow-inner">
                       <div className="text-[9px] text-muted-foreground/20 font-black uppercase tracking-[0.5em] italic leading-none">Target_Vault_Entry</div>
                       <div className="text-xl font-black font-mono text-primary uppercase italic truncate leading-none">{diplomatStats.solAddress ? diplomatStats.solAddress : 'SYNCING_CLUSTER...'}</div>
                    </div>
                 </div>
              </div>

              {/* REMOTE QUANTUM HUD */}
              <div className={`p-12 border-2 rounded-[5rem] backdrop-blur-3xl space-y-12 shadow-[0_80px_150px_rgba(0,0,0,1)] transition-all relative overflow-hidden group ${remoteQuantum.connected ? 'border-primary/30 bg-primary/5 shadow-[0_40px_100px_rgba(var(--primary),0.1)]' : 'border-red-500/10 bg-red-500/5'}`}>
                 ...
                 <div className="flex flex-col gap-8 items-start relative z-10 pl-2">
                   ...
                   <div className="p-6 bg-muted/40 border-2 border-border rounded-[2rem] shadow-inner group-hover:border-primary/20 transition-all flex flex-col gap-2">
                      <div className="text-[10px] text-muted-foreground/20 font-black uppercase tracking-[0.5em] italic leading-none">Operation_Registry</div>
                      <div className={`text-xl font-black font-mono uppercase italic leading-none ${remoteQuantum.connected ? 'text-foreground' : 'text-red-500/40'}`}>{remoteQuantum.lastJobId || 'WAITING_CMD'}</div>
                   </div>
                 </div>
              </div>
           </div>
                   ...
                   {agents.map((agent) => (
                      <motion.div 
                        ...
                        className={`p-16 border-2 rounded-[5rem] backdrop-blur-3xl flex flex-col gap-12 transition-all relative overflow-hidden shadow-2xl group ${
                          agent.status === 'READING' ? 'bg-primary/5 border-primary/40 shadow-[0_40px_80px_rgba(var(--primary),0.15)]' : 'bg-muted border-border hover:border-border'
                        }`}
                      >
                        ...
                        <div className="flex justify-between items-start relative z-10 w-full">
                          <div className="flex items-center gap-10">
                            ...
                            <div className="space-y-4 pt-2">
                              <div className="font-black text-foreground text-4xl italic tracking-tighter uppercase leading-none">{agent.name}</div>
                              <div className="text-[12px] text-primary font-black uppercase tracking-[0.4em] italic leading-none">{agent.specialty}</div>
                            </div>
                          </div>
                          <div className={`px-6 py-2.5 rounded-full text-[11px] font-black uppercase tracking-[0.4em] italic border-2 transition-all leading-none ${
                            agent.status === 'READING' ? 'bg-primary text-primary-foreground border-primary shadow-2xl animate-pulse' :
                            agent.status === 'PROCESSING' ? 'bg-white text-background border-border shadow-2xl' :
                            'bg-muted/10 text-muted-foreground/20 border-border overflow-hidden'
                          }`}>
                            {agent.status}
                          </div>
                        </div>
                        ...
                        <div className="flex-1 space-y-12 relative z-10">
                          {agent.article && (
                            <div className="bg-muted/60 border-2 border-border rounded-[3.5rem] p-12 space-y-6 shadow-2xl group-hover:border-primary/20 transition-all">
                              <div className="flex items-center gap-4 text-[12px] font-black text-primary/60 uppercase tracking-[0.5em] italic leading-none pl-1">
                                <Terminal size={18} strokeWidth={3} /> {agent.article.source}
                              </div>
                              <div className="text-3xl font-black text-foreground leading-[1.1] italic tracking-tighter line-clamp-2 pl-1 group-hover:text-foreground transition-colors">
                                "{agent.article.title}"
                              </div>
                            </div>
                          )}
                          ...
                          <div className="space-y-4 px-2">
                            <div className="flex justify-between text-[12px] text-muted-foreground/20 font-black uppercase tracking-[0.6em] italic leading-none pl-1">
                              <span>Neural Ingest Progress</span>
                              <span className="text-primary animate-pulse">{agent.article?.progress || 0}%</span>
                            </div>
                            <div className="h-4 bg-muted border-2 border-border rounded-full overflow-hidden shadow-inner p-[2px]">
                              <motion.div 
                                className="h-full bg-primary shadow-[0_0_30px_rgba(var(--primary),0.4)] rounded-full"
                        ...
                        <div className="flex justify-between items-center text-[12px] border-t-2 border-border mt-4 pt-10 relative z-10 px-4">
                          <div className="flex flex-wrap items-center gap-10 font-black uppercase tracking-[0.4em] italic leading-none">
                            <span className="text-muted-foreground/10 flex items-center gap-4 group-hover:text-muted-foreground/20 transition-colors"><Binary size={20} strokeWidth={2.5}/> {agent.articlesRead} READS</span>
                            <span className="text-primary flex items-center gap-4 group-hover:scale-105 transition-all"><Sparkles size={20} strokeWidth={2.5}/> {agent.knowledgePoints} RESONANCE</span>
                          </div>
                          <div className="w-12 h-12 rounded-2xl border-2 border-border flex items-center justify-center text-muted-foreground/10 group-hover:border-primary/40 group-hover:text-primary transition-all cursor-crosshair">
              ...
              <div className="bg-background border-2 border-border rounded-[5rem] overflow-hidden shadow-[0_80px_150px_rgba(0,0,0,1)]">
                 <div className="p-12 lg:px-16 border-b-2 border-border flex flex-col md:flex-row md:items-end justify-between gap-12 bg-muted/10">
                    <div className="space-y-6">
                       ...
                       <div className="flex items-center gap-6 pl-4">
                          <div className="px-6 py-2 bg-muted border-2 border-border rounded-full text-[11px] font-black uppercase tracking-[0.5em] italic text-muted-foreground/20 leading-none">STRATUM_V1.OMEGA</div>
                          <div className="px-6 py-2 bg-primary/10 border-2 border-primary/20 rounded-full text-[11px] font-black uppercase tracking-[0.5em] italic text-primary leading-none animate-pulse">QUANTUM_OPTIMIZED</div>
                       </div>
                    </div>
                 ...
                 <div className="p-12 lg:p-16 grid grid-cols-1 md:grid-cols-2 gap-12">
                   {miningStats.workers.map(worker => (
                      <div key={worker.id} className="p-8 md:p-14 bg-muted border-2 border-border responsive-rounded space-y-8 md:space-y-12 shadow-2xl group hover:border-primary/40 transition-all backdrop-blur-3xl relative overflow-hidden flex flex-col justify-between h-auto md:h-[320px] min-h-[250px]">
                         <div className="absolute top-0 right-0 p-12 opacity-[0.01] group-hover:scale-150 transition-transform duration-3000">
                            <Binary size={250} className="text-primary" />
                         </div>
                         
                         <div className="flex justify-between items-center relative z-10 w-full">
                            <div className="text-2xl font-black text-muted-foreground/60 uppercase italic tracking-tighter group-hover:text-foreground transition-colors leading-none pl-1">{worker.name}</div>
                            <div className="flex items-center gap-4 bg-muted/10 border-2 border-border px-6 py-2.5 rounded-full">
                              <div className={`w-3 h-3 rounded-full animate-pulse ${worker.name.includes('Quantum') ? 'bg-primary shadow-[0_0_20px_hsl(var(--primary))]' : 'bg-primary/20'}`} />
                              <span className={`text-[10px] font-black uppercase tracking-[0.4em] italic leading-none ${worker.name.includes('Quantum') ? 'text-primary' : 'text-muted-foreground/10'}`}>
                         ...
                         <div className="flex justify-between items-end relative z-10 border-t-2 border-border pt-10 px-1">
                            <div className="space-y-4">
                              <div className="text-[11px] text-muted-foreground/10 font-black uppercase tracking-[0.5em] italic leading-none group-hover:text-muted-foreground/20 transition-colors">
                                {worker.name.includes('Quantum') ? 'Optimizations' : 'Ledger_Shares'}
                              </div>
                              <div className={`text-4xl font-black italic tracking-tighter leading-none ${worker.name.includes('Quantum') ? 'text-primary' : 'text-muted-foreground/90'}`}>{worker.shares}</div>
                            </div>
                            <div className="w-16 h-16 bg-white shadow-2xl rounded-2xl flex items-center justify-center text-background group-hover:bg-primary transition-all border-4 border-border group-hover:scale-110 active:scale-95">
        ...
        <section className="pt-40 text-center space-y-16">
            ...
            <Link href="/" className="inline-flex items-center gap-8 text-[12px] font-black uppercase tracking-[1rem] text-muted-foreground/10 hover:text-primary transition-all italic group active:scale-95 leading-none pl-4">
