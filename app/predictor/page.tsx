  ...
  return (
    <div className="relative min-h-screen bg-background text-foreground selection:bg-primary/40 font-sans overflow-x-hidden pb-40 overflow-x-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-[-10%] left-[-10%] w-[90vw] h-[90vw] bg-primary/5 blur-[250px] rounded-full" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[70vw] h-[70vw] bg-primary/2 blur-[200px] rounded-full" />
        <div className="absolute inset-0 bg-[url('/assets/noise.png')] opacity-[0.03] mix-blend-overlay dark:opacity-[0.05]" />
      </div>

      <header className="relative z-10 w-full p-8 lg:px-14 flex justify-between items-center bg-background/40 backdrop-blur-3xl border-b border-border">
        <Link href="/" className="inline-flex items-center gap-4 text-muted-foreground/50 hover:text-primary transition-all text-[11px] font-black uppercase tracking-[0.5em] group italic active:scale-95">
        ...
        <div className="space-y-16">
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            >
              ...
              <div className="flex items-center gap-6 relative z-10">
                <div className="h-14 w-14 bg-primary/10 border border-primary/20 rounded-2xl flex items-center justify-center text-primary shadow-2xl group-hover:scale-110 transition-transform">
                   <Terminal size={28} />
                </div>
                <div>
                   <h3 className="text-2xl font-black uppercase italic tracking-tighter">Neural Seed Material</h3>
                   <p className="text-[10px] text-muted-foreground/50 uppercase font-black tracking-[0.4em] italic mt-1 font-mono">INPUT_STREAM_v7.0</p>
                </div>
              </div>
              <textarea 
                id="predictor-seed"
            ...
            <div className="p-8 md:p-12 border border-border bg-muted/5 responsive-rounded space-y-10 group hover:border-primary/30 transition-all shadow-2xl relative overflow-hidden backdrop-blur-xl shadow-inner">
              ...
              <div className="flex items-center gap-6 relative z-10">
                ...
                <div>
                   ...
                   <p className="text-[10px] text-muted-foreground/50 uppercase font-black tracking-[0.4em] italic mt-1 font-mono">SWARM_MODIFIERS</p>
                </div>
              </div>
                ...
                <div className="flex flex-wrap gap-4 min-h-[140px] content-start">
                  {variables.map((v, i) => (
                    <motion.span 
                        ...
                        className="px-6 py-3 bg-primary/10 border border-primary/20 text-primary text-[11px] rounded-2xl font-black uppercase tracking-[0.2em] flex items-center gap-4 italic group/pill hover:border-primary/50 transition-all shadow-xl"
                    >
                       {v} <button onClick={() => setVariables(variables.filter((_, idx) => idx !== i))} className="hover:text-foreground transition-colors opacity-40 hover:opacity-100 flex items-center justify-center p-1 bg-muted/20 rounded-full"><X size={12} /></button>
                    </motion.span>
                  ))}
                  {variables.length === 0 && (
          ...
          <div className="flex justify-center">
            <button 
              ...
              className="group relative bg-primary text-primary-foreground px-32 py-10 rounded-[3rem] font-black uppercase tracking-[0.6em] shadow-[0_40px_100px_rgba(var(--primary),0.3)] hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-20 disabled:grayscale overflow-hidden italic text-lg shadow-2xl"
            >
        ...
        <AnimatePresence>
          {(isSimulating || predictionResult) && (
            <motion.div 
              ...
              className="w-full"
            >
              <div className="bg-background border border-border responsive-rounded p-8 md:p-16 lg:p-24 relative overflow-hidden backdrop-blur-3xl shadow-[0_80px_150px_rgba(0,0,0,0.9)] group">
                
                {/* Progress Tracking */}
                {isSimulating && (
                  <div className="absolute top-0 left-0 h-3 bg-primary transition-all duration-1000 shadow-[0_0_50px_var(--primary)] z-50" style={{ width: `${progress}%` }} />
                )}
                ...
                <div className="grid lg:grid-cols-12 gap-24 relative z-10">
                  ...
                  <div className="lg:col-span-4 space-y-16 flex flex-col justify-between">
                     <div className="space-y-12">
                       <div className="flex items-center gap-8">
                          <div className="h-20 w-20 rounded-[2rem] bg-muted/20 border border-primary/30 flex items-center justify-center text-primary shadow-[0_20px_40px_rgba(var(--primary),0.2)] group-hover:scale-110 transition-transform duration-700">
                             <Cpu size={40} />
                          </div>
                          <div>
                             <h3 className="text-3xl font-black uppercase tracking-tighter italic leading-none">Swarm Indices</h3>
                             <p className="text-[10px] text-primary uppercase tracking-[0.4em] font-black italic mt-3 animate-pulse">TELEMETRY_SYNC_v4.1</p>
                          </div>
                       </div>
                       
                       <div className="space-y-8 divide-y divide-border">
                          <div className="flex justify-between items-center py-6">
                             <span className="text-[11px] font-black uppercase text-muted-foreground/50 tracking-[0.4em] italic leading-none">Projection ID</span>
                             <span className="font-black text-xs text-primary uppercase tracking-tighter italic">{predictionResult?.id || 'HANDSHAKE...'}</span>
                          </div>
                          <div className="flex justify-between items-center py-6">
                             <span className="text-[11px] font-black uppercase text-muted-foreground/50 tracking-[0.4em] italic leading-none">Confidence Index</span>
                             <span className="font-black italic text-primary text-4xl tracking-tighter leading-none">{predictionResult ? (predictionResult.confidence * 100).toFixed(0) : '—'}%</span>
                          </div>
                          <div className="flex justify-between items-center py-6">
                             <span className="text-[11px] font-black uppercase text-muted-foreground/50 tracking-[0.4em] italic leading-none">Iterative Rounds</span>
                             <span className="font-black text-2xl uppercase italic text-foreground tracking-tighter leading-none">{predictionResult?.rounds || '...'}</span>
                          </div>
                       </div>
                     </div>
                     
                     <div className="p-10 bg-muted/10 border border-border rounded-[3rem] space-y-6 relative overflow-hidden shadow-inner">
                  ...
                  {/* Report Output Area */}
                  <div className="lg:col-span-8 bg-muted/10 responsive-rounded border border-border p-8 md:p-16 lg:p-24 flex flex-col relative shadow-2xl overflow-hidden group/report">
                     ...
                     <div className="flex-1 text-3xl text-foreground/80 font-light leading-[1.6] whitespace-pre-wrap selection:bg-primary/40 italic relative z-10">
                        {isSimulating ? (
                          <div className="flex flex-col gap-10">
                             <div className="h-10 w-full bg-muted rounded-3xl animate-pulse" />
                          ...
                          <div className="prose prose-invert prose-2xl max-w-none prose-p:text-muted-foreground/80 prose-p:italic prose-p:font-light prose-headings:font-black prose-headings:uppercase prose-headings:italic prose-headings:tracking-tighter prose-strong:text-primary">
                     ...
                     {!isSimulating && predictionResult && (
                       <div className="mt-20 flex flex-col md:flex-row justify-between items-center gap-8 bg-primary/5 p-10 rounded-[3rem] border border-primary/20 shadow-2xl backdrop-blur-3xl relative overflow-hidden group/sync">
                          <div className="absolute inset-0 bg-gradient-to-r from-primary/5 via-transparent to-transparent opacity-50" />
                          <div className="space-y-3 relative z-10">
                             <div className="text-[11px] font-black text-muted-foreground uppercase tracking-[0.6em] italic leading-none">Protocol Handshake</div>
                             <div className="text-[11px] font-black text-primary uppercase tracking-[0.4em] italic leading-none">OMEGA_BROADCAST: PENDING_USER_SIG</div>
                          </div>
                          <button className="px-10 py-5 bg-primary text-primary-foreground rounded-2xl text-[11px] font-black uppercase tracking-[0.4em] italic flex items-center gap-4 hover:scale-110 active:scale-95 transition-all shadow-2xl shadow-[var(--primary)]/30">
      ...
      {/* BACKGROUND DECOR */}
      <div className="fixed bottom-0 right-0 p-12 opacity-[0.02] pointer-events-none select-none z-0 text-foreground">
          <div className="text-fluid-hero font-black italic leading-none uppercase">PREDICT</div>
      </div>
