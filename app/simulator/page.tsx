...
function MeshBroadcastRipple({ index }: { index: number }) {
  ...
  return (
    <mesh ref={ref}>
       ...
       <meshBasicMaterial color="hsl(var(--primary))" transparent wireframe opacity={0.1} />
    </mesh>
  );
}
...
function NeuralPactLine({ index }: { index: number }) {
  ...
  return (
     <Line points={pts} color="hsl(var(--primary))" lineWidth={0.5} transparent opacity={0.1} dashed dashScale={10} gapSize={10} />
  );
}
  ...
  return (
    <group position={pos}>
       ...
       >
          ...
          <meshBasicMaterial color={hovered ? "hsl(var(--foreground))" : "hsl(var(--primary))"} transparent opacity={0.9} />
          {hovered && (
             <Html distanceFactor={10} position={[0, 0.6, 0]}>
                <div className="bg-background border border-primary/40 p-5 rounded-[2rem] whitespace-nowrap text-[11px] font-black tracking-[0.4em] text-primary backdrop-blur-3xl pointer-events-none uppercase italic shadow-[0_20px_50px_rgba(0,0,0,0.8)] border-2">
...
function NeuralShardPulse({ index, label }: { index: number, label: string }) {
  ...
  return (
    <Float speed={3} rotationIntensity={2} floatIntensity={3} position={pos}>
       ...
       <Text position={[0, -0.3, 0]} fontSize={0.1} color="hsl(var(--primary))" anchorX="center" maxWidth={2.5} textAlign="center" opacity={0.3}>
          {label}
       </Text>
    </Float>
  );
...
export default function SimulatorPage() {
  ...
  return (
    <div className="relative h-screen w-screen bg-background text-foreground selection:bg-primary/40 font-sans overflow-hidden flex">
      ...
      <div className="absolute inset-0 z-0 cursor-move">
        <Canvas shadows gl={{ antialias: true, stencil: false, depth: true }} dpr={[1, 2]}>
          ...
          <pointLight position={[25, 25, 25]} intensity={2} color="hsl(var(--primary))" />
          <pointLight position={[-25, -25, -25]} intensity={1} color="hsl(var(--primary))" />
          <spotLight position={[0, 20, 0]} angle={0.3} penumbra={1} intensity={1} color="hsl(var(--foreground))" castShadow />
      ...
      <div className="relative z-10 w-full h-full pointer-events-none flex flex-col justify-between p-6 md:p-8 lg:p-14">
        ...
        <header className="flex justify-between items-start pointer-events-auto">
           <div className="space-y-8">
              <Link href="/" className="inline-flex items-center gap-4 text-muted-foreground/20 hover:text-primary transition-all text-[11px] font-black uppercase tracking-[0.6em] mb-4 group italic">
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex gap-12 pt-6">
                     <div className="inline-flex items-center gap-4 text-[11px] font-black text-primary uppercase tracking-[0.6em] italic">
                       <Orbit size={18} className="animate-spin-slow" /> Network Resonance: 100%
                     </div>
                     <div className="inline-flex items-center gap-4 text-[11px] font-black text-muted-foreground/10 uppercase tracking-[0.4em] font-mono italic">
           ...
           <div className="flex flex-col items-end space-y-8">
              <div className="bg-background/95 border border-border p-8 md:p-12 lg:p-16 responsive-rounded backdrop-blur-3xl min-w-[300px] md:min-w-[350px] lg:min-w-[500px] space-y-8 md:space-y-12 shadow-[0_40px_100px_rgba(0,0,0,0.8)] transition-all hover:border-primary/30 group">
                 <div className="flex justify-between items-center">
                    <div className="text-[11px] text-primary uppercase tracking-[0.6em] flex items-center gap-4 italic font-black">
                 ...
                 <div className="space-y-8 max-h-[250px] overflow-y-auto pr-6 custom-scrollbar">
                    {meshPeers.map(peer => (
                      <div key={peer.id} className="flex justify-between items-center group/peer hover:bg-muted/10 p-4 rounded-2xl transition-all">
                         <div className="flex flex-col gap-1">
                            <span className="text-lg font-black text-muted-foreground/80 group-hover/peer:text-foreground transition-colors italic uppercase tracking-tighter">{peer.name}</span>
                            <span className="text-[10px] font-mono text-muted-foreground/20 uppercase tracking-[0.2em]">{peer.url}</span>
                         </div>
                         <div className="flex items-center gap-6">
                            <div className="text-right">
                                <span className="text-[11px] font-black text-primary tabular-nums italic">{peer.resilience}%</span>
                                <div className="text-[8px] font-black text-muted-foreground/10 uppercase tracking-widest">Stability</div>
                            </div>
                            <div className={`h-4 w-4 rounded-xl ${peer.status === 'ONLINE' ? 'bg-primary shadow-[0_0_20px_rgba(var(--primary),0.6)]' : 'bg-amber-400 animate-pulse shadow-[0_0_15px_rgba(251,191,36,0.4)]'}`} />
                         </div>
                      </div>
                    ))}
                    {meshPeers.length === 0 && (
                        <div className="py-12 text-center text-[10px] font-black uppercase tracking-[0.5em] text-muted-foreground/5 italic">Searching for proximal mesh peers...</div>
                    )}
                 </div>
              </div>
              ...
              {selectedNode && (
                 <motion.div 
                   ...
                   className="bg-primary text-primary-foreground p-12 rounded-[3.5rem] backdrop-blur-3xl min-w-[380px] space-y-8 shadow-[0_40px_80px_rgba(var(--primary),0.3)] relative overflow-hidden group border-4 border-border"
                 >
                    ...
                    <div className="pt-8 border-t border-border flex justify-between items-center">
                       ...
                       <div className="w-10 h-10 bg-muted/10 rounded-full flex items-center justify-center animate-spin-slow">
                          <Radio size={16} />
                       </div>
                    </div>
                 </motion.div>
        ...
        <footer className="flex flex-col lg:flex-row justify-between items-end gap-16 pointer-events-auto">
           <div className="w-full lg:max-w-3xl bg-background/95 border border-border responsive-rounded p-8 md:p-12 lg:p-16 backdrop-blur-3xl overflow-hidden relative group shadow-[0_50px_120px_rgba(0,0,0,0.9)] transition-all hover:bg-muted group">
              <div className="absolute top-0 right-0 p-16 opacity-[0.02]">
                 <Terminal size={180} className="text-primary group-hover:scale-110 transition-transform duration-1000" />
              </div>
              <div className="flex items-center justify-between mb-12">
                  <div className="flex items-center gap-6 text-[12px] font-black uppercase tracking-[0.6em] text-muted-foreground/20 italic leading-none">
                     <ShieldCheck size={24} className="text-primary animate-pulse" /> Transmission Ledger (OMEGA_SYNC)
                  </div>
                  <div className="text-[10px] font-mono text-primary/40 italic">ENCRYPTION: AES-256-Ω</div>
              </div>
                 ...
                 <AnimatePresence mode="popLayout">
                    {telemetry.map((log, i) => (
                      <motion.div 
                        ...
                        className="font-mono text-[12px] text-primary mb-4 uppercase italic tracking-[0.2em] flex items-center gap-6"
                      >
           ...
           <div className="flex gap-10">
              <button 
                ...
                className="h-32 w-32 bg-muted/10 border border-border flex items-center justify-center rounded-[3.5rem] text-muted-foreground/10 hover:text-primary hover:border-primary/40 transition-all shadow-2xl active:scale-90 group relative overflow-hidden"
              >
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <motion.aside 
            ...
            className="fixed right-0 top-0 bottom-0 w-full md:w-[600px] bg-background/98 border-l border-border z-[100] backdrop-blur-[50px] overflow-y-auto p-8 md:p-20 custom-scrollbar shadow-[-50px_0_150px_rgba(0,0,0,0.9)]"
          >
            <div className="space-y-32">
               <div className="space-y-16">
                  <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => setSidebarOpen(false)} className="text-muted-foreground/10 hover:text-primary transition-all bg-muted/10 p-4 rounded-2xl hover:scale-110 active:scale-90 border border-border">
                       <X size={32} />
                    </button>
                  </div>
                  ...
                  <div className="space-y-8">
                     {activeShards.length > 0 ? activeShards.map((shard, i) => (
                       <motion.div 
                         ...
                         className={`p-8 md:p-10 border responsive-rounded space-y-6 md:space-y-8 hover:border-primary/50 transition-all cursor-pointer group shadow-2xl relative overflow-hidden ${
                           selectedNode?.label === shard.label ? 'bg-primary text-primary-foreground border-primary shadow-[0_20px_60px_rgba(var(--primary),0.4)]' : 'bg-muted/10 border-border hover:bg-muted/10'
                         }`}
                         onClick={() => handleNodeSelect(shard.id, shard.label)}
                       >
                          <div className={`flex justify-between items-center text-[11px] font-black uppercase tracking-[0.4em] italic leading-none ${selectedNode?.label === shard.label ? 'text-background/40' : 'text-muted-foreground/20'}`}>
                             <div className="flex items-center gap-4">
                                <div className={`w-2 h-2 rounded-full ${selectedNode?.label === shard.label ? 'bg-muted animate-pulse' : 'bg-primary animate-pulse'}`} />
               ...
               <div className="space-y-16 pt-32 border-t border-border relative overflow-hidden group">
                  <motion.div animate={{ x: [-200, 600] }} transition={{ duration: 6, repeat: Infinity, ease: 'linear' }} className="absolute h-1 w-full bg-gradient-to-r from-transparent via-primary/30 to-transparent top-0" />
                  ...
                  <div className="flex justify-between items-end">
                      ...
                      <div className="text-right">
                         <div className="text-5xl font-black italic tracking-tighter text-foreground">99.9%</div>
                         <div className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/10 mt-1">Uptime Global</div>
                      </div>
                  </div>

                  <div className="grid grid-cols-2 gap-12 pt-8">
                    <div className="p-8 bg-muted/10 border border-border rounded-[2.5rem] space-y-4">
                       <div className="text-[11px] font-black text-muted-foreground/20 uppercase tracking-[0.4em] italic leading-none">P2P Latency</div>
                       <div className="text-3xl font-black text-primary italic tracking-tighter leading-none">8ms</div>
                    </div>
                    <div className="p-8 bg-muted/10 border border-border rounded-[2.5rem] space-y-4 flex flex-col justify-between">
                       <div className="text-[11px] font-black text-muted-foreground/20 uppercase tracking-[0.4em] italic leading-none">Grid Sync</div>
                       <div className="flex gap-1.5 items-end h-8">
                          {[0.7, 1, 0.8, 0.95, 0.6, 1, 0.75].map((h, i) => (
                            <motion.div key={i} animate={{ height: [`${h*100}%`, '100%', `${h*60}%`] }} transition={{ duration: 2.5, repeat: Infinity, delay: i * 0.15 }} className="w-2 bg-primary/30 rounded-full" />
                          ))}
                       </div>
                    </div>
                  </div>
