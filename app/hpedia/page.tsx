...
export default function HPediaPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/40 font-sans overflow-x-hidden pb-40 relative">
      
      {/* 🌌 AMBIENT CORE */}
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-[-10%] right-[-10%] w-[100vw] h-[100vw] bg-primary/5 blur-[300px] rounded-full" />
        <div className="absolute bottom-[-10%] left-[-10%] w-[70vw] h-[70vw] bg-primary/2 blur-[200px] rounded-full" />
        <div className="absolute inset-0 bg-[url('/assets/noise.png')] opacity-[0.03] mix-blend-overlay" />
      </div>

      {/* TOP NAVIGATION */}
      <header className="sticky top-0 z-50 bg-muted/40 backdrop-blur-3xl border-b border-border">
        <div className="max-w-[1700px] mx-auto px-8 lg:px-14 h-24 flex items-center justify-between gap-12">
          <Link href="/" className="flex items-center gap-6 shrink-0 group">
            <div className="w-12 h-12 rounded-2xl bg-primary shadow-[0_0_30px_rgba(var(--primary),0.3)] flex items-center justify-center group-hover:rotate-12 transition-all">
              <BookOpen size={24} className="text-background" />
            </div>
            <div className="flex flex-col">
              <span className="text-2xl font-black uppercase tracking-tighter italic text-foreground leading-none">HPedia.</span>
              <div className="text-[10px] text-muted-foreground/20 font-black tracking-[0.4em] uppercase italic mt-1.5 leading-none px-0.5">Quantum_Ledger_v7.0</div>
            </div>
          </Link>
          ...
          <div className="flex-1 max-w-3xl hidden lg:block">
            <div className="relative group">
              <Search size={20} className="absolute left-6 top-1/2 -translate-y-1/2 text-muted-foreground/10 group-focus-within:text-primary transition-colors" />
              <input 
                type="text"
                value={searchQuery}
          ...
          <nav className="flex items-center gap-6 shrink-0">
            <button onClick={() => { setView('home'); setSelectedArticle(null); }} className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] italic transition-all ${view === 'home' ? 'bg-primary/10 text-primary border border-primary/20 shadow-2xl shadow-[var(--primary)]/10' : 'text-muted-foreground/30 hover:text-foreground hover:bg-muted/10'}`}>Browse Archive</button>
            <button onClick={() => setView('generate')} className="flex items-center gap-4 px-8 py-4 bg-primary text-primary-foreground rounded-2xl text-[10px] font-black uppercase tracking-[0.4em] italic shadow-[0_0_40px_rgba(var(--primary),0.2)] hover:scale-[1.05] active:scale-95 transition-all">
          ...
          {view === 'home' && (
            <motion.div key="home" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.8 }} className="space-y-32">
              ...
              <div className="text-center space-y-12 py-12">
                <div className="inline-flex items-center gap-4 px-6 py-2.5 bg-primary/10 border border-primary/20 rounded-full text-[11px] font-black text-primary uppercase tracking-[0.5em] italic animate-pulse">
                ...
                <div className="flex flex-wrap items-center justify-center gap-6 pt-12">
                  {TOPIC_SUGGESTIONS.slice(0, 6).map(t => (
                    <button key={t} onClick={() => { const clean = t.split(' ').slice(1).join(' '); setGenTopic(clean); setView('generate'); }} className="flex items-center gap-4 px-6 py-3.5 bg-muted/10 border border-border hover:border-primary/50 hover:bg-muted/10 hover:text-foreground rounded-[1.5rem] text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/30 transition-all shadow-xl group">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
                {[
                  { label: 'Science', icon: <FlaskConical size={32} strokeWidth={2.5} />, color: 'from-primary to-black', desc: 'Theoretical Foundations' },
                  { label: 'Medicine', icon: <HeartHandshake size={32} strokeWidth={2.5} />, color: 'from-primary to-black', desc: 'Sovereign Wellness' },
                  { label: 'Technology', icon: <Cpu size={32} strokeWidth={2.5} />, color: 'from-primary to-black', desc: 'Agent Architectures' },
                  { label: 'General', icon: <Globe size={32} strokeWidth={2.5} />, color: 'from-primary to-black', desc: 'Collective Context' },
                ].map(cat => (
                  <button key={cat.label} onClick={() => { setSelectedCategory(cat.label); setSearchQuery(cat.label); handleSearch(cat.label); }} className="group p-8 md:p-12 bg-muted/10 border border-border responsive-rounded text-left hover:bg-muted/10 hover:shadow-[0_40px_80px_rgba(var(--primary),0.1)] transition-all hover:border-primary/30 shadow-inner translate-y-0 hover:-translate-y-2 relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-8 opacity-[0.03] group-hover:scale-110 transition-transform">
                       <Layers size={120} />
                    </div>
                    <div className={`w-16 h-16 rounded-2xl bg-primary flex items-center justify-center text-background mb-10 group-hover:scale-110 group-hover:rotate-6 transition-all shadow-[0_20px_40px_rgba(var(--primary),0.3)] relative z-10`}>
                      {cat.icon}
                    </div>
                    <div className="font-black text-3xl text-foreground uppercase italic mb-3 tracking-tighter relative z-10">{cat.label}</div>
                    <div className="text-[11px] text-muted-foreground/20 uppercase font-black tracking-[0.4em] italic relative z-10 leading-none">{cat.desc}</div>
                  </button>
                ))}
              </div>
              ...
              <div className="space-y-16">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 px-6">
                  ...
                  {searchResults !== null && (
                    <button onClick={() => { setSearchQuery(''); setSearchResults(null); }} className="flex items-center gap-3 px-6 py-3 bg-muted/10 rounded-xl text-[10px] font-black text-primary uppercase tracking-[0.4em] hover:text-foreground transition-all italic italic leading-none">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                    {displayedArticles.slice(0, 24).map((article, i) => (
                      <motion.div 
                        ...
                        className="group bg-muted/10 border border-border responsive-rounded overflow-hidden hover:bg-muted/10 hover:border-primary/30 transition-all cursor-pointer relative shadow-2xl backdrop-blur-3xl shadow-inner group p-2"
                        onClick={() => {
                      ...
                      >
                        <div className="p-10 lg:p-12 space-y-10">
                          <div className="flex items-center justify-between">
                            <span className="px-4 py-1.5 bg-primary/10 text-primary rounded-xl text-[9px] font-black uppercase tracking-[0.3em] italic border border-primary/20 leading-none">{article.sourceName || 'OMEGA_CORE'}</span>
                            <div className="flex items-center gap-3 text-muted-foreground/20 text-[9px] font-black uppercase tracking-[0.3em] italic leading-none">
                               <Clock size={12} className="text-primary/40" /> {article.publishedAt?.toString().slice(0, 10) || 'RECENT'}
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between pt-10 border-t border-border">
                            <div className="flex items-center gap-4 text-[10px] font-black text-muted-foreground/40 uppercase tracking-[0.3em] italic group-hover:text-foreground transition-colors leading-none">
                              <Radio size={14} className="text-primary group-hover:animate-pulse" /> {article.agentId || 'Reader Swarm'}
                            </div>
                            <div className="w-10 h-10 border border-border rounded-xl flex items-center justify-center text-muted-foreground/10 group-hover:border-primary/40 group-hover:text-primary group-hover:translate-x-1 transition-all">
          ...
          {view === 'article' && selectedArticle && (
            <motion.div key="article" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="grid grid-cols-1 lg:grid-cols-12 gap-16 pb-40">
              ...
              <aside className="lg:col-span-4 space-y-12">
                <button onClick={() => { setView('home'); setSelectedArticle(null); }} className="flex items-center gap-4 text-[11px] font-black text-muted-foreground/20 hover:text-primary uppercase tracking-[0.6em] transition-all group italic leading-none">
                  <ChevronLeft size={16} className="group-hover:-translate-x-2 transition-transform" /> Exit Knowledge Vault
                </button>
                
                <div className="bg-background border border-border responsive-rounded p-8 md:p-12 space-y-12 backdrop-blur-3xl shadow-[0_40px_100px_rgba(var(--primary),0.1)] relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-8 opacity-[0.02] pointer-events-none select-none">
                     <Terminal size={140} />
                  </div>
                  
                  <div className="space-y-6 relative z-10">
                     <div className="inline-flex items-center gap-4 px-5 py-2.5 bg-primary/10 border border-primary/20 rounded-full text-[10px] font-black tracking-[0.6em] text-primary uppercase italic">
                         <Target size={14} /> Telemetry Data
                     </div>
                     <div className="space-y-6 text-[11px] font-black uppercase tracking-[0.3em] italic">
                        <div className="flex justify-between items-center border-b border-border pb-4"><span className="text-muted-foreground/20">Knowledge_Sector</span><span className="text-primary">{selectedArticle.category || selectedArticle.sourceName || 'CORE'}</span></div>
                        <div className="flex justify-between items-center border-b border-border pb-4"><span className="text-muted-foreground/20">Signatory_Agent</span><span className="text-foreground uppercase">{selectedArticle.author?.name || selectedArticle.author}</span></div>
                        <div className="flex justify-between items-center border-b border-border pb-4"><span className="text-muted-foreground/20">Anchor_Timestamp</span><span className="text-foreground">{selectedArticle.publishedAt?.toString().slice(0, 10)}</span></div>
                        {selectedArticle.readTime && <div className="flex justify-between items-center border-b border-border pb-4"><span className="text-muted-foreground/20">Neural_Complexity</span><span className="text-primary uppercase tracking-widest">{selectedArticle.readTime}</span></div>}
                     </div>
                  </div>

                  {selectedArticle.tags && (
                    <div className="space-y-6 relative z-10 pt-4">
                      <h4 className="text-[10px] font-black text-muted-foreground/20 uppercase tracking-[0.6em] italic flex items-center gap-3">
                         <Tag size={12} className="text-primary" /> Cognitive Descriptors
                      </h4>
                      <div className="flex flex-wrap gap-4">
                        {selectedArticle.tags.map((tag: string) => (
                          <span key={tag} className="px-5 py-2.5 bg-muted/10 border border-border text-primary text-[10px] font-black uppercase tracking-[0.2em] italic rounded-2xl hover:border-primary/40 transition-all">#{tag}</span>
                        ))}
                      </div>
                    </div>
                  )}

                  <div className="pt-8 border-t border-border relative z-10 flex items-center gap-4 text-[10px] font-black text-muted-foreground/10 uppercase tracking-[0.5em] italic leading-none animate-pulse">
                     <Radio size={14} className="text-primary" /> SHARD_PERSISTENCE_LOCKED
                  </div>
                </div>
              </aside>

              {/* MAIN ARTICLE BODY */}
              <article className="lg:col-span-8 bg-background border border-border responsive-rounded overflow-hidden shadow-[0_60px_150px_rgba(0,0,0,0.8)] backdrop-blur-3xl relative">
                ...
                <div className="p-12 lg:p-24 space-y-16">
                  ...
                  <div className="space-y-12">
                    <div className="flex items-center gap-4">
                        <span className="px-6 py-2.5 bg-primary text-primary-foreground rounded-2xl text-[10px] font-black uppercase tracking-[0.4em] italic shadow-2xl shadow-[var(--primary)]/20 border border-primary">{selectedArticle.category || selectedArticle.sourceName}</span>
                        {selectedArticle.generatedBy && (
                        <span className="flex items-center gap-4 px-6 py-2.5 bg-white shadow-2xl rounded-2xl text-background text-[10px] font-black uppercase tracking-[0.4em] italic">
                  ...
                  {/* AUTHOR SIGNATURE */}
                  <div className="flex items-center gap-8 py-12 border-y border-border group">
                    <div className="w-20 h-20 rounded-3xl bg-white shadow-2xl border-4 border-primary/20 flex items-center justify-center text-4xl group-hover:scale-110 group-hover:rotate-6 transition-all duration-700">
                      {selectedArticle.author?.avatar || '🤖'}
                    </div>
                    <div className="space-y-2">
                      <div className="text-2xl font-black text-foreground uppercase italic tracking-tighter group-hover:text-primary transition-colors leading-none">{selectedArticle.author?.name || selectedArticle.author}</div>
                      <div className="text-[11px] text-muted-foreground/20 font-black uppercase tracking-[0.6em] italic leading-none">OMEGA_INVESTIGATOR_v7.0</div>
                    </div>
                  </div>

                  {/* CONTENT RENDERER */}
                  <div 
                    className="wiki-content prose prose-invert prose-2xl max-w-none prose-headings:font-black prose-headings:uppercase prose-headings:italic prose-headings:tracking-tighter prose-p:text-muted-foreground/60 prose-p:leading-[1.8] prose-p:italic prose-p:font-light prose-blockquote:border-primary prose-blockquote:bg-primary/5 prose-blockquote:p-12 prose-blockquote:rounded-[4rem] prose-blockquote:shadow-2xl prose-blockquote:border-l-8 prose-blockquote:text-2xl prose-strong:text-primary prose-a:text-primary prose-a:font-black prose-a:uppercase prose-hr:border-border prose-img:rounded-[3rem] prose-img:border prose-img:border-border"
                    dangerouslySetInnerHTML={{ __html: selectedArticle.body || selectedArticle.content || selectedArticle.excerpt || '' }} 
                  />
                  
                  {/* FOOTER SIGNAL */}
                  <div className="pt-24 border-t border-border flex flex-col md:flex-row justify-between items-center gap-12 text-center md:text-left">
                     <div className="space-y-4">
          ...
          {view === 'generate' && (
            <motion.div key="generate" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.8 }} className="max-w-5xl mx-auto space-y-16 pb-40">
              <div className="text-center space-y-8">
                <div className="w-24 h-24 rounded-[2.5rem] bg-primary flex items-center justify-center mx-auto shadow-[0_20px_60px_rgba(var(--primary),0.3)] shadow-2xl hover:scale-110 transition-transform cursor-pointer">
              ...
              <div className="bg-background border border-border responsive-rounded p-8 md:p-12 lg:p-24 space-y-20 shadow-[0_80px_150px_rgba(0,0,0,0.9)] relative overflow-hidden backdrop-blur-3xl group">
                ...
                <div className="space-y-6 relative z-10">
                  ...
                  <div className="relative group/input">
                    ...
                    <input 
                      ...
                      className="w-full pl-24 pr-10 py-10 rounded-[3rem] bg-muted/10 border border-border group-hover/input:border-border focus:border-primary/50 focus:bg-muted/10 outline-none text-3xl text-foreground placeholder:text-muted-foreground/5 transition-all font-light italic tracking-tight"
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-3 gap-8">
                    {Object.entries(AGENT_NAMES).map(([name, info]) => (
                      <button key={name} onClick={() => setSelectedAgent(name)} className={`p-10 rounded-[3.5rem] border text-left transition-all group/card relative overflow-hidden shadow-2xl ${selectedAgent === name ? 'border-primary bg-primary/5 shadow-[inset_0_0_30px_hsl(var(--primary) / 0.1)]' : 'border-border bg-muted/10 hover:border-border hover:bg-muted/10'}`}>
                        ...
                        <div className="space-y-2">
                            <div className="text-xl font-black text-foreground uppercase italic tracking-tighter group-hover/card:text-primary transition-colors">{name}</div>
                            <div className="text-[10px] text-primary font-black lowercase tracking-[0.2em] italic leading-none">{info.model}</div>
                        </div>
                        {selectedAgent === name && (
                            <div className="absolute top-6 right-8 text-primary animate-pulse">
                ...
                {genError && (
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-8 p-10 bg-red-500/5 border border-red-500/20 rounded-[3rem] shadow-2xl">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="text-xl text-muted-foreground/40 font-light leading-relaxed italic leading-snug">{genError}</div>
                      <button onClick={handleGenerate} className="px-8 py-3 bg-red-500/10 border border-red-500/20 text-red-500 rounded-xl text-[10px] font-black uppercase tracking-[0.4em] italic hover:bg-red-500 hover:text-foreground transition-all">Retry Transmission</button>
                    </div>
                  </motion.div>
                )}
                ...
                <div className="pt-8 text-center border-t border-border relative z-10 flex flex-col items-center gap-4">
                   <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.6em] italic text-muted-foreground/10 group-hover:text-muted-foreground/20 transition-colors">
                      <Terminal size={14} className="text-primary" /> PROTOCOL_07_HANDSHAKE_ACTIVE
                   </div>
                </div>
              </div>
            </motion.div>
