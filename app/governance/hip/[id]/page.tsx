...
export default function ProtocolDetail({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (loading) return <div className="min-h-screen bg-muted flex items-center justify-center text-foreground font-black uppercase tracking-widest italic animate-pulse">Establishing Secure Handshake...</div>;
    ...
    return (
        <div className="relative min-h-screen bg-background text-foreground selection:bg-primary/40 font-sans overflow-x-hidden pb-40">
            ...
            <main className="relative z-10 max-w-[1700px] mx-auto px-8 pt-24 lg:pt-32 flex flex-col lg:flex-row gap-16 lg:gap-32 items-start">
                ...
                <div className="flex-1 space-y-16 lg:sticky lg:top-32">
                    ...
                    >
                        ...
                        <div className="flex flex-wrap items-center gap-10">
                             <div className="p-8 bg-muted/10 border-2 border-border rounded-[2.5rem] flex items-center gap-6 backdrop-blur-3xl group hover:border-primary/30 transition-all shadow-2xl">
                                 <div className="h-14 w-14 bg-muted border-2 border-border rounded-2xl flex items-center justify-center text-primary group-hover:scale-110 transition-transform">
                                     <Fingerprint size={28} strokeWidth={2.5} />
                                 </div>
                                 <div className="space-y-1">
                                    <div className="text-[10px] text-muted-foreground/20 uppercase tracking-[0.4em] font-black italic">PROPOSAL_AUTHOR</div>
                                    <div className="text-xl font-black text-foreground italic tracking-tight">{hip.authorId}</div>
                                 </div>
                             </div>
                             <div className="p-8 bg-muted/10 border-2 border-border rounded-[2.5rem] flex items-center gap-6 backdrop-blur-3xl group hover:border-primary/30 transition-all shadow-2xl">
                                 <div className="h-14 w-14 bg-muted border-2 border-border rounded-2xl flex items-center justify-center text-primary group-hover:scale-110 transition-transform">
                                 ...
                                 <div className="space-y-1">
                                    <div className="text-[10px] text-muted-foreground/20 uppercase tracking-[0.4em] font-black italic">STATUS_SYNC</div>
                                    <div className="text-xl font-black text-primary italic tracking-tight animate-pulse">{hip.status}</div>
                                 </div>
                             </div>
                        </div>

                        <div className="prose prose-invert prose-headings:font-black prose-headings:tracking-tighter prose-headings:italic prose-headings:uppercase prose-headings:text-muted-foreground/60 prose-p:text-2xl prose-p:text-muted-foreground/40 prose-p:font-light prose-p:leading-relaxed prose-p:italic prose-strong:text-primary prose-strong:font-black prose-li:text-xl prose-li:text-muted-foreground/30 prose-li:font-light prose-li:italic prose-hr:border-border max-w-4xl py-12 border-t border-border font-sans selection:bg-primary/40">
                ...
                <div className="w-full lg:w-[480px] shrink-0 pb-40 lg:sticky lg:top-32">
                    <motion.div 
                        ...
                        className="bg-background border-2 border-border rounded-[5rem] p-12 lg:p-16 backdrop-blur-[80px] space-y-16 relative overflow-hidden shadow-[0_80px_150px_rgba(0,0,0,1)] group"
                    >
                        <div className="absolute top-0 right-0 p-12 opacity-[0.01] group-hover:scale-110 transition-transform duration-1000">
                           <Network size={250} className="text-primary rotate-12" />
                        </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="h-3 w-full bg-muted/10 rounded-full overflow-hidden border border-border p-1">
                                        <motion.div 
                                            ...
                                            className="h-full bg-gradient-to-r from-primary/40 to-primary shadow-[0_0_20px_hsl(var(--primary))]"
                                        />
                                    </div>
                                    <div className="flex justify-between text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/10 italic px-2">
                        ...
                        <div className="pt-8 space-y-8 relative z-10 border-t-2 border-border">
                            <div className="space-y-4">
                                    ...
                                    {!voted ? (
                                        <motion.div key="vote" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="space-y-6">
                                            ...
                                            <div 
                                                ...
                                                className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all cursor-pointer select-none ${ghostMode ? 'bg-primary/10 border-primary/40' : 'bg-muted/10 border-border opacity-50 hover:opacity-100'}`}
                                            >
                                                <div className="flex items-center gap-4">
                                                    <div className={`p-2 rounded-xl ${ghostMode ? 'bg-primary text-primary-foreground' : 'bg-muted/10 text-muted-foreground/40'}`}>
                                                        <ShieldHalf size={16} strokeWidth={3} />
                                                    </div>
                                                    <div className="flex flex-col">
                                                        <span className={`text-[10px] font-black uppercase tracking-[0.2em] italic leading-none ${ghostMode ? 'text-primary' : 'text-muted-foreground/40'}`}>Ghost Mode</span>
                                                        <span className="text-[8px] text-muted-foreground/20 font-black uppercase tracking-[0.1em] italic">Anonymize Identity</span>
                                                    </div>
                                                </div>
                                                <div className={`w-8 h-4 rounded-full relative transition-colors ${ghostMode ? 'bg-primary' : 'bg-muted/10'}`}>
                                ...
                                <p className="text-[11px] text-center text-muted-foreground/20 font-black uppercase tracking-[0.5em] italic leading-relaxed pt-2">
                                    Verification: <span className="text-muted-foreground/40">Broadcasting 1.0 Signal via Sovereign Identity. Gasless settlement.</span>
                                </p>
                            </div>
                        </div>

                        {/* LIVE VOTE FEED */}
                        <div className="space-y-8 pt-8 border-t-2 border-border relative z-10">
                            <div className="flex items-center justify-between">
                                <h4 className="text-[11px] font-black uppercase tracking-[0.6em] text-muted-foreground/10 italic pl-2">Signal Log_</h4>
                                <span className="text-[10px] text-primary/40 font-black uppercase tracking-[0.4em] italic">{hip.voteCount ?? 0} signals</span>
                            </div>

                            {/* Resonance breakdown */}
                            {hip.resonanceBreakdown && (
                                <div className="space-y-4 p-6 bg-muted/40 border border-border rounded-3xl">
                                    {[{ label: 'Support', val: hip.resonanceBreakdown.support, color: 'hsl(var(--primary))' }, { label: 'Against', val: hip.resonanceBreakdown.against, color: '#ef4444' }, { label: 'Abstain', val: hip.resonanceBreakdown.abstain, color: '#ffffff30' }].map(bar => {
                                        ...
                                        return (
                                            <div key={bar.label} className="space-y-1">
                                                <div className="flex justify-between text-[9px] font-black uppercase tracking-[0.4em] italic">
                                                    ...
                                                    <span className="text-muted-foreground/20">{bar.val.toFixed(1)}</span>
                                                </div>
                            ...
                            <div className="space-y-3 max-h-48 overflow-y-auto custom-scrollbar">
                                {!hip.votes || hip.votes.length === 0 ? (
                                    <p className="text-center text-[11px] text-muted-foreground/10 font-black uppercase italic tracking-widest py-6">No signals broadcast yet.</p>
                                ) : hip.votes.map((vote: any, i: number) => (
                                    <div key={vote.id ?? i} className="flex justify-between items-center p-5 bg-muted/10 border border-border rounded-2xl group hover:border-primary/30 transition-all">
                                        <div className="flex items-center gap-4">
                                            <div className="flex flex-col gap-1">
                                                <span className="text-[11px] font-black text-muted-foreground/60 group-hover:text-foreground uppercase italic tracking-tight leading-none flex items-center gap-2">
                                                    {vote.voterId?.length > 16 ? `${vote.voterId.slice(0,8)}...${vote.voterId.slice(-4)}` : vote.voterId}
                                                    {vote.isGhost && <ShieldHalf size={10} className="text-primary" />}
                                                </span>
                                                <span className={`text-[9px] font-black uppercase tracking-[0.4em] italic leading-none ${vote.choice === 'Support' ? 'text-primary' : vote.choice === 'Against' ? 'text-red-400' : 'text-muted-foreground/20'}`}>{vote.choice}</span>
                                            </div>
                                        </div>
                                        <span className="text-[10px] text-muted-foreground/10 font-black uppercase tracking-[0.2em] italic">{new Date(vote.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            <style jsx global>{`
                ...
                .prose strong { color: hsl(var(--primary)) !important; }
            `}</style>
        </div>
    );
}
