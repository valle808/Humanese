...
export default function NotFound() {
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/40 font-sans overflow-hidden flex flex-col items-center justify-center p-8 relative">
      ...
      <main className="relative z-10 w-full max-w-4xl text-center space-y-20">
        ...
        <div className="space-y-12">
            <motion.div 
               ...
               className="inline-flex items-center gap-6 px-8 py-3 bg-primary/10 border border-primary/30 rounded-full backdrop-blur-3xl shadow-[0_30px_60px_rgba(var(--primary),0.1)]"
            >
                <AlertTriangle size={24} className="text-primary animate-pulse" strokeWidth={2.5} />
                <span className="text-[12px] font-black tracking-[0.6em] text-primary uppercase italic leading-none">Status: 404_NULL_COGNITION</span>
            </motion.div>
        ...
        <motion.div 
           ...
           className="p-8 lg:p-14 bg-muted/60 border border-border responsive-rounded text-left space-y-8 backdrop-blur-3xl shadow-[0_80px_150px_rgba(0,0,0,1)] relative overflow-hidden group"
        >
           <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-primary/40 to-transparent" />
           <div className="flex items-center justify-between border-b border-border pb-6">
              <div className="flex items-center gap-4 text-[11px] font-black uppercase tracking-[0.5em] text-muted-foreground/20 italic leading-none">
                 <Terminal size={16} className="text-primary" /> Diagnostic_Extraction
              </div>
              <div className="h-2 w-2 rounded-full bg-primary/40 animate-pulse" />
           </div>
           
           <div className="space-y-4 font-mono text-[11px] lg:text-[13px] text-muted-foreground/30 uppercase tracking-widest leading-loose italic">
              <div>[00.00] initializing_void_sweep...</div>
              <div>[00.04] tracing_coordinate_path: <span className="text-primary">FAILED</span></div>
              <div>[00.12] abyssal_mesh_scan: <span className="text-primary">NO_RESULTS_MATCH</span></div>
              <div>[00.18] cognitive_index_status: <span className="text-primary">UNREACHABLE</span></div>
              <div className="text-muted-foreground/60 animate-pulse pt-4">ALERT: SECTOR_RESTRICTED_OR_PURGED</div>
           </div>
        </motion.div>
