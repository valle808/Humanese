            ...
            if (Array.isArray(history) && history.length > 0) {
                historyHTML = history.map((msg: any) => {
                    ...
                    return (
                        ...
                        '<div style="font-size: 11px; font-weight: 900; letter-spacing: 2px; color: ' + roleColor + '; margin-bottom: 12px; text-transform: uppercase; font-family: monospace;">' + roleName + '</div>' +
                        '<div style="font-size: 15px; line-height: 1.7; color: #e0e0e0;">' + textContent + imageContent + '</div>' +
                        '</div>'
                    );
                }).join('');
            }
...
<body>
    ...
    ${historyHTML ? '<div class="section-title">&gt;_ NEURAL_LOG_EXTRACT: CONVERSATION HISTORY</div><div class="history-container">' + historyHTML + '</div>' : ''}
