  ...
  return (
    <div className="relative min-h-screen bg-background text-foreground font-sans selection:bg-primary/40 selection:text-foreground overflow-x-hidden pb-40">
        ...
        <section className="grid lg:grid-cols-2 gap-16 items-start">
            <div className="space-y-12 h-full flex flex-col justify-center">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <button onClick={triggerQuantumSync} disabled={isSyncing} className="p-8 md:p-10 bg-background border-2 border-border responsive-rounded text-left space-y-6 hover:border-primary/40 transition-all group shadow-inner backdrop-blur-3xl active:scale-95 leading-none">
                      <div className="w-20 h-20 bg-primary/10 border-2 border-primary/20 rounded-[2rem] flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-background transition-all group-hover:scale-110">
                          <Activity size={32} className={isSyncing ? 'animate-spin' : ''} strokeWidth={3} />
                      </div>
                      <div className="space-y-2">
                        <div className="text-[12px] font-black text-foreground uppercase tracking-[0.4em] italic group-hover:text-primary transition-colors">Japan Quantum Node</div>
                        <p className="text-[10px] text-muted-foreground/10 font-bold uppercase tracking-widest italic leading-tight">Handshake Bridge Protocol</p>
                      </div>
                   </button>

                   <button onClick={triggerSwarmOverdrive} disabled={isGenerating} className="p-8 md:p-10 bg-primary border-2 border-primary/20 responsive-rounded text-left space-y-6 hover:shadow-[0_40px_100px_rgba(var(--primary),0.3)] transition-all group active:scale-95 leading-none border-0">
                      <div className="w-20 h-20 bg-muted/10 border-2 border-border rounded-[2rem] flex items-center justify-center text-background group-hover:scale-110 transition-transform">
                          {isGenerating ? <Loader2 size={32} className="animate-spin" strokeWidth={3} /> : <Sparkles size={32} className="animate-pulse" strokeWidth={3} />}
                      </div>
                      <div className="space-y-2">
                        <div className="text-[12px] font-black text-background uppercase tracking-[0.4em] italic">Trigger Swarm Overdrive</div>
                        <p className="text-[10px] text-background/40 font-bold uppercase tracking-widest italic leading-tight">Initialize Investigative Neural Net</p>
                      </div>
                   </button>
                </div>
                
                <div className="p-8 md:p-12 bg-background border-2 border-border responsive-rounded space-y-6 md:space-y-8 shadow-inner backdrop-blur-3xl group hover:border-primary/20 transition-all">
                   <div className="flex items-center gap-6">
                      <div className="w-16 h-16 bg-muted/10 border-2 border-border rounded-[2rem] flex items-center justify-center text-muted-foreground/20 group-hover:text-primary transition-all">
                        <Share2 size={24} strokeWidth={3} />
                      </div>
                      <div className="space-y-1">
                        <h3 className="text-2xl font-black uppercase italic tracking-tighter leading-none text-muted-foreground/40 group-hover:text-foreground transition-colors">Collaborate with Monroe</h3>
                   ...
                   <Link href="/monroe" className="inline-flex items-center gap-6 text-[11px] font-black uppercase tracking-[0.6em] text-primary hover:text-foreground transition-all italic leading-none group/link">
                      Initialize Link Shard <ArrowRight size={18} className="group-hover/link:translate-x-3 transition-transform" strokeWidth={3} />
                   </Link>
                </div>
            </div>

            <div className="relative bg-background border-2 border-border rounded-[5rem] overflow-hidden shadow-[0_80px_150px_rgba(0,0,0,1)] group h-[600px] lg:h-[700px] shadow-inner">
               ...
               <div className="absolute inset-0 flex flex-col items-center justify-center space-y-12 relative z-20">
                  <div className="relative">
                     ...
                     <div className="relative p-16 bg-muted border-2 border-border rounded-full shadow-[0_40px_100px_rgba(var(--primary),0.2)] group-hover:border-primary/40 transition-all duration-700">
                        <Atom size={120} className="text-primary/40 animate-[spin_20s_linear_infinite]" strokeWidth={1} />
                        <Dna size={50} className="absolute inset-0 m-auto text-primary animate-pulse" strokeWidth={2.5} />
                     </div>
                  </div>
               ...
               <div className="flex flex-wrap gap-4">
                  {['ALL', 'Medicine', 'Energy', 'Quantum', 'Physics'].map(cat => (
                    <button 
                      ...
                      className={`px-8 py-3 rounded-full text-[11px] font-black uppercase tracking-[0.4em] transition-all duration-300 italic border-2 leading-none active:scale-95 ${activeCategory === cat ? 'bg-primary text-primary-foreground border-primary shadow-[0_20px_40px_rgba(var(--primary),0.3)]' : 'bg-muted/10 hover:bg-primary/5 text-muted-foreground/20 hover:text-foreground hover:border-primary/40 border-border'}`}
                    >
                      {cat}
                    </button>
                  ))}
            ...
            <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-12">
               ...
               {loading ? (
                 Array.from({ length: 6 }).map((_, i) => (
                   <div key={i} className="h-[450px] bg-background border-2 border-border rounded-[5rem] animate-pulse shadow-inner" />
                 ))
               ) : articles.length === 0 ? (
                 ...
                 articles.map((article, i) => (
                   ...
                   >
                      ...
                      <div className="flex justify-between items-start relative z-20">
                          <div className="p-10 rounded-[2.5rem] bg-muted border-2 border-border group-hover:border-primary/40 shadow-inner group-hover:bg-primary/5 transition-all text-muted-foreground/20 group-hover:text-primary">
                      ...
                      <div className="pt-10 border-t-2 border-border flex items-center justify-between text-[11px] font-black uppercase tracking-[0.4em] italic relative z-20 leading-none">
                         <div className="flex items-center gap-4 text-primary/40 group-hover:text-primary transition-colors">
                            <Activity size={18} strokeWidth={3} /> {article.sourceName}
                         </div>
                         <div className="text-muted-foreground/5 group-hover:text-muted-foreground/20 transition-colors">{new Date(article.publishedAt).toLocaleDateString()}</div>
                      </div>
      ...
      <div className="fixed bottom-0 left-0 p-16 opacity-[0.01] pointer-events-none select-none z-0">
          <div className="text-fluid-hero font-black italic leading-none uppercase">PROGRESS</div>
      </div>
