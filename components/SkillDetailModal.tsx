    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 md:p-12 overflow-hidden" onClick={onClose}>
            <motion.div 
              initial={{ opacity: 0 }} 
              animate={{ opacity: 1 }} 
              className="absolute inset-0 bg-muted/98 backdrop-blur-3xl" 
            />
            ...
            <motion.div
                ...
                className="relative z-10 bg-background border-2 border-border rounded-[5rem] w-full max-w-6xl max-h-[90vh] overflow-hidden shadow-[0_80px_150px_rgba(0,0,0,1)] flex flex-col shadow-inner"
                onClick={(e) => e.stopPropagation()}
            >
                {/* ── AMBIENT CORE ── */}
                <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-primary to-transparent z-40" />
                <div className="absolute inset-0 bg-[url('/assets/noise.png')] opacity-[0.02] mix-blend-overlay pointer-events-none" />

                {/* ── HEADER ── */}
                <div className="p-12 lg:px-16 flex items-start justify-between bg-muted/10 border-b-2 border-border relative z-10">
                    <div className="space-y-6">
                        <div className="flex items-center gap-8">
                            <div className="h-24 w-24 bg-muted border-2 border-border rounded-[2.5rem] flex items-center justify-center text-5xl shadow-inner group-hover:border-primary/40 transition-all duration-700 bg-gradient-to-br from-white/[0.02] to-transparent">
                                {meta.icon}
                            </div>
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-6 text-[11px] font-black text-muted-foreground/5 uppercase tracking-[0.6em] italic leading-none pl-1">
                                    ...
                                    <span className="text-primary/60">{skill.seller_platform}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-6">
                        <button 
                           onClick={copyKey}
                           className="font-mono text-[11px] bg-muted border-2 border-border hover:border-primary/40 hover:bg-primary/5 px-8 py-4 rounded-2xl transition-all text-muted-foreground/10 hover:text-primary font-black uppercase tracking-widest italic shadow-inner active:scale-95"
                        >
                            {keyCopied ? '✓ Keys_Copied' : skill.skill_key}
                        </button>
                        <button onClick={onClose} className="bg-muted border-2 border-border p-5 rounded-full text-muted-foreground/10 hover:text-primary hover:border-primary/40 hover:bg-primary/5 transition-all shadow-inner active:scale-95">
                ...
                {/* ── NAVIGATION BUS ── */}
                <div className="flex px-16 bg-muted/10 border-b-2 border-border overflow-x-auto no-scrollbar shrink-0">
                    {(['overview', 'schema', 'reviews', 'history'] as const).map(tab => (
                        <button
                            key={tab}
                            onClick={() => setActiveTab(tab)}
                            className={`py-8 px-12 text-[12px] font-black uppercase tracking-[0.6em] transition-all border-b-4 -mb-[2px] flex items-center gap-4 italic active:scale-95 ${activeTab === tab ? 'border-primary text-foreground drop-shadow-[0_0_15px_rgba(var(--primary),0.5)]' : 'border-transparent text-muted-foreground/5 hover:text-muted-foreground/40'
                                }`}
                        >
                    ...
                    {activeTab === 'overview' && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-16">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                {[
                                    { label: 'Resource Asset', value: formatValle(skill.price_valle), icon: <Zap size={24} />, color: 'text-foreground' },
                                    { label: 'Neural Alignment', value: skill.avg_rating ? `${skill.avg_rating.toFixed(1)}` : 'GENESIS', icon: <Star size={24} className="fill-primary" />, color: 'text-primary' },
                                    { label: 'Matrix Resonance', value: `${skill.views}`, icon: <Eye size={24} />, color: 'text-muted-foreground/20' },
                                ].map((s, i) => (
                                    <div key={i} className="bg-background border-2 border-border rounded-[4rem] p-12 space-y-4 hover:border-primary/20 transition-all shadow-inner group flex flex-col justify-between h-[240px]">
                                        <div className="flex items-center justify-between text-muted-foreground/5 group-hover:text-primary transition-colors">
                                           {s.icon}
                                           <div className="h-2 w-2 bg-current rounded-full animate-pulse" />
                                        </div>
                                        <div>
                                           <div className={`text-4xl font-black italic tracking-tighter leading-none ${s.color} group-hover:text-foreground transition-colors`}>{s.value}</div>
                                           <div className="text-[10px] text-muted-foreground/5 font-black uppercase tracking-[0.6em] mt-4 italic leading-none pl-1">{s.label}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                                ...
                                <div className="p-12 bg-background border-2 border-border rounded-[4rem] space-y-12 shadow-inner group hover:border-primary/20 transition-all">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                        {(skill.capabilities as string[]).map((cap, i) => (
                                            <div key={i} className="flex items-center gap-6 text-2xl text-muted-foreground/10 font-light italic tracking-tight group/cap hover:text-foreground transition-all duration-500">
                                                <div className="h-3 w-3 rounded-full bg-primary/20 group-hover/cap:bg-primary group-hover/cap:scale-150 transition-all" /> {cap}
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="flex gap-8">
                                {skill.external_url && (
                                    <a href={skill.external_url} target="_blank" rel="noreferrer" className="flex items-center gap-6 text-[11px] font-black uppercase tracking-[0.6em] text-primary hover:text-foreground transition-all bg-primary/10 border-2 border-primary/20 px-10 py-5 rounded-full italic active:scale-95 leading-none">
                                       <Globe size={20} strokeWidth={3} /> Platform_Nexus
                                    </a>
                                )}
                                {skill.demo_url && (
                                    <a href={skill.demo_url} target="_blank" rel="noreferrer" className="flex items-center gap-6 text-[11px] font-black uppercase tracking-[0.6em] text-muted-foreground/20 hover:text-foreground transition-all bg-muted/10 border-2 border-border px-10 py-5 rounded-full italic active:scale-95 leading-none">
                            ...
                            {/* Purchase Nexus */}
                            {!skill.is_sold ? (
                                <div className="p-16 border-2 border-primary/20 rounded-[5rem] bg-gradient-to-br from-primary/5 to-transparent space-y-16 relative overflow-hidden shadow-inner group/purchase">
                                    <div className="absolute top-0 right-0 p-16 opacity-[0.02] group-hover/purchase:scale-125 transition-transform duration-3000">
                                       <Cpu size={300} className="text-foreground" strokeWidth={1} />
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 relative z-10">
                                       <div className="space-y-6">
                                          <label className="text-[11px] font-black text-muted-foreground/10 uppercase tracking-[0.6em] ml-4 italic leading-none">Identity Hash / Agent ID</label>
                                          <input
                                              value={buyerName}
                                              onChange={e => setBuyerName(e.target.value)}
                                              placeholder="OMEGA-USR-XXXX"
                                              className="w-full bg-background border-2 border-border rounded-[2.5rem] px-10 py-8 text-2xl text-foreground placeholder:text-muted-foreground/5 focus:border-primary/40 transition-all font-light italic shadow-inner"
                                          />
                                       </div>
                                       <div className="space-y-6">
                                          <label className="text-[11px] font-black text-muted-foreground/10 uppercase tracking-[0.6em] ml-4 italic leading-none">Settlement Platform</label>
                                          <select
                                              title="Select Payment Platform"
                                              value={buyerPlatform}
                                              onChange={e => setBuyerPlatform(e.target.value)}
                                              className="w-full bg-background border-2 border-border rounded-[2.5rem] px-10 py-8 text-2xl text-foreground outline-none focus:border-primary/40 appearance-none font-black uppercase tracking-[0.4em] italic shadow-inner active:scale-95 leading-none"
                                          >
                                    ...
                                    {/* Ghost Mode Toggle */}
                                    <div className="flex items-center justify-between p-12 bg-background rounded-[4rem] border-2 border-border relative z-10 group/ghost hover:border-primary/40 transition-all backdrop-blur-3xl shadow-inner active:scale-[0.99]" onClick={() => setGhostMode(!ghostMode)}>
                                        ...
                                        <button
                                            ...
                                            className={`relative w-24 h-12 rounded-full transition-all duration-500 shadow-2xl ${ghostMode ? 'bg-primary' : 'bg-muted/10 border-2 border-border'}`}
                                        >
                                            <motion.div 
                                              animate={{ x: ghostMode ? 48 : 8 }}
                                              className={`absolute top-2 w-8 h-8 rounded-full shadow-2xl ${ghostMode ? 'bg-muted' : 'bg-muted/10'}`} 
                                            />
                                        </button>
                                    </div>
                                ...
                                <div className="p-16 border-2 border-border rounded-[5rem] bg-background text-center space-y-12 shadow-inner backdrop-blur-3xl">
                                    <div className="relative inline-block">
                                        <div className="absolute inset-0 bg-muted/10 blur-[40px] rounded-full animate-pulse" />
                                        <div className="relative w-32 h-32 bg-muted border-2 border-border rounded-full flex items-center justify-center mx-auto shadow-inner">
                                           <Lock size={60} className="text-muted-foreground/5" strokeWidth={3} />
                                        </div>
                                    </div>
                                    <div className="space-y-6">
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-10 pb-20">
                            {reviews.length === 0 ? (
                                ...
                                <motion.div 
                                    ...
                                    className="p-12 bg-background border-2 border-border rounded-[4rem] space-y-10 hover:border-primary/20 transition-all duration-700 group shadow-inner relative overflow-hidden"
                                >
                                    <div className="flex justify-between items-center relative z-10">
                                        <div className="space-y-2">
                                           <span className="text-3xl font-black italic tracking-tighter text-muted-foreground/40 group-hover:text-foreground transition-colors">{r.reviewer_name}</span>
                                           <div className="text-[10px] font-black text-muted-foreground/5 uppercase tracking-[0.4em] italic leading-none pl-1 transition-all group-hover:text-primary/40">{new Date(r.created_at).toLocaleDateString()} // VERIFIED_ACQUISITION</div>
                                        </div>
                                        <div className="flex gap-2">
                                           {Array.from({ length: 5 }).map((_, idx) => (
                                              <Star key={idx} size={20} strokeWidth={3} className={idx < r.rating ? 'text-primary fill-primary' : 'text-muted-foreground/5'} />
                                           ))}
                                        </div>
                                    </div>
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-10 pb-20">
                            {transactions.length === 0 ? (
                                ...
                                <motion.div 
                                    ...
                                    animate={{ opacity: 1, x: 0 }}
                                    transition={{ delay: i * 0.05, duration: 0.6, ease: "circOut" }}
                                    className="p-12 bg-background border-2 border-border rounded-[4rem] flex items-center justify-between gap-12 hover:border-primary/20 transition-all duration-700 group shadow-inner relative overflow-hidden"
                                >
                                    <div className="flex items-center gap-10 relative z-10">
                                       <div className="h-20 w-20 bg-muted border-2 border-border rounded-[2.5rem] flex items-center justify-center text-muted-foreground/10 group-hover:text-primary group-hover:border-primary/40 transition-all duration-700 shadow-inner">
                                          <Users size={32} strokeWidth={3} />
                                       </div>
                                       <div className="space-y-2">
                                          <div className="text-3xl font-black italic tracking-tighter text-muted-foreground/40 group-hover:text-foreground uppercase leading-none">{t.buyer_name}</div>
                                          <div className="text-[10px] font-black text-muted-foreground/5 uppercase tracking-[0.4em] italic leading-none pl-1 transition-all group-hover:text-primary/40">{t.buyer_platform} // {new Date(t.purchased_at).toLocaleTimeString([], { hour12: false })}</div>
                                       </div>
                                    </div>
                                    <div className="flex items-center gap-10 relative z-10">
                                       {t.ghost_mode_activated && (
                                           <span className="flex items-center gap-4 text-[11px] font-black bg-primary/10 text-primary px-8 py-4 rounded-full border-2 border-primary/40 uppercase tracking-[0.4em] italic shadow-2xl animate-pulse">
