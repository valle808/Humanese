...
export function SkillListingForm({ onClose, onSuccess }: SkillListingFormProps) {
    ...
    const inputClass = 'w-full bg-background border-2 border-border rounded-2xl px-8 py-4 text-lg text-foreground placeholder:text-muted-foreground/5 outline-none focus:border-primary/40 focus:bg-primary/5 transition-all duration-500 font-light italic shadow-inner';
    const labelClass = 'text-[11px] font-black uppercase tracking-[0.4em] text-muted-foreground/10 mb-3 block italic pl-4';
    ...
    return (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 md:p-12 overflow-hidden" onClick={onClose}>
            ...
            <motion.div
                ...
                className="relative z-10 bg-background border-2 border-border rounded-[5rem] w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-[0_80px_150px_rgba(0,0,0,1)] flex flex-col shadow-inner"
            ...
            >
                ...
                <div className="p-12 lg:px-16 flex items-start justify-between bg-muted/10 border-b-2 border-border relative z-10">
                    <div className="space-y-4">
                        <div className="flex items-center gap-8">
                            <div className="h-20 w-20 bg-primary rounded-[2rem] flex items-center justify-center text-background shadow-[0_0_30px_hsl(var(--primary))]">
                ...
                <div className="flex-1 overflow-y-auto p-12 lg:p-16 custom-scrollbar space-y-16">
                    {/* ── PREVIEW KEY ── */}
                    <div className="p-12 bg-primary/5 border-2 border-primary/20 rounded-[4rem] group hover:border-primary/50 transition-all shadow-inner relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-12 opacity-[0.05] group-hover:scale-125 transition-transform duration-2000">
                           <ShieldCheck size={120} className="text-primary" strokeWidth={1} />
                        </div>
                        <div className="space-y-6 relative z-10">
                            <div className="flex items-center gap-6">
                               <div className="w-12 h-12 bg-primary rounded-2xl flex items-center justify-center text-background">
                                  <Binary size={24} strokeWidth={3} />
                               </div>
                               <p className="text-[12px] font-black text-primary/60 uppercase tracking-[0.5em] italic leading-none">Sovereign Skill Key (Auto-Generated)</p>
                            </div>
                            <p className="font-mono text-4xl font-black text-foreground italic tracking-[0.2em] leading-none">{previewKey}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-8 pb-32">
                        <button onClick={onClose} className="flex-1 bg-muted border-2 border-border rounded-[2.5rem] py-10 text-[12px] font-black uppercase tracking-[0.5em] text-muted-foreground/10 hover:text-foreground hover:border-border transition-all italic active:scale-95 shadow-inner leading-none">
                            Abort_Directive
                        </button>
                        <button 
