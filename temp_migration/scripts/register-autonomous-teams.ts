...
async function main() {
  ...
  // 1. Find or create the Master User (Sergio Valle)
  let masterUser = await prisma.user.findFirst({
    where: { OR: [{ email: 'sergio@humanese.com' }, { id: 'SergioValle' }] }
  });

  if (!masterUser) {
    masterUser = await prisma.user.findFirst(); // Fallback to first user
  }
